...
import { FaRocket, FaLock, FaArrowRight, FaEye, FaEyeSlash, FaCheckCircle, FaTimesCircle, FaInfoCircle, FaCreditCard, FaTimes } from 'react-icons/fa'; // Added more icons
...
const TrialPage = () => {
  ...
  // --- Eligibility Check ---
  const checkEligibility = useCallback(async () => { // Wrap in useCallback
    setIsChecking(true);
    setIsEligible(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!user) {
        if (!email || !password || !firstName || !lastName) { // Check names too
          throw new Error('Please fill in all required fields: Email, Password, First Name, Last Name.');
        }
        ...
        const registerResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/auth/register`, {
          method: 'POST',
        ...
        if (!registerResponse.ok) {
          ...
          throw new Error(registerBody.message || registerBody.error || 'Account registration failed.');
        }
        ...
        console.log("Attempting auto-login after registration...");
         try {
              ...
              const loginResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/v1/auth/login`, {
                   method: 'POST',
                   headers: { 'Content-Type': 'application/json' },
              ...
              if (!loginResponse.ok) {
                   throw new Error(loginBody.message || loginBody.error || 'Auto-login failed after registration.');
              }
              ...
              // Depending on flow, you might stop here or try to proceed without token
               throw new Error("Auto-login failed after registration. Cannot proceed with trial setup without authentication."); // Safer to stop
         }
      }
      // === END STEP 1 & 2 ===
      ...
      if (!subResponse.ok) {
        ...
        throw new Error(subBody.message || subBody.error || `Failed to start trial: ${subResponse.status}`);
      }
          ...
          <div className="min-h-screen w-full flex items-center justify-center text-matrix-green  p-4 ">
            <div className="text-center max-w-2xl mx-auto bg-black/50 border-2 border-matrix-green rounded-xl p-8 shadow-lg">
              ...
              <p className="mb-6 text-matrix-green/80">
                  Looks like you may already have an active subscription or have used a free trial previously.
              </p>
    ...
    <div className="min-h-screen w-full text-matrix-green bg-black p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div
          ...
          <form onSubmit={handleSubmit} className="bg-black/40 border-2 border-matrix-green rounded-xl p-8 space-y-6 shadow-lg">
            ...
            {!user && (
              <>
                 ...
                 <div className="space-y-2">
                     <label htmlFor="trial-firstName" className="block text-matrix-green font-medium">First Name</label>
                     <input
                         id="trial-firstName"
                         type="text"
                         value={firstName}
                 ...
                 {/* Last Name */}
                 <div className="space-y-2">
                     <label htmlFor="trial-lastName" className="block text-matrix-green font-medium">Last Name</label>
                     <input
                         id="trial-lastName"
                         type="text"
                         value={lastName}
                ...
                {/* Email */}
                <div className="space-y-2">
                  <label htmlFor="trial-email" className="block text-matrix-green font-medium">Email Address</label>
                  <input
                    id="trial-email"
                    type="email"
                    value={email}
                ...
                {/* Password */}
                <div className="space-y-2">
                  <label htmlFor="trial-password" className="block text-matrix-green font-medium">Create Password</label>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-matrix-green/60 pt-1">Card required for trial activation, but won't be charged now.</p>
            </div>

            {/* Error Display */}
            {error && (
              <motion.div /* ... */ className="text-red-400 ...">{error}</motion.div>
            )}
            ...
            >
              {isProcessing ? 'Processing...' : `Start ${STRIPE_CONFIG.freeTrialDays}-Day Free Trial`}
            </motion.button>
