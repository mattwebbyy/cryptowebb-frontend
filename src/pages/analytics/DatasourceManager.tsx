...
const DatasourceManager = () => {
    ...
    const handleDelete = (id: string, name: string) => {
        if (window.confirm(`Are you sure you want to delete the datasource "${name}"? This cannot be undone.`)) {
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-6">
                ...
                <Button
                    onClick={() => { /* TODO: Open create modal/form */ alert("Open 'Create Datasource' form/modal"); }}
            ...
            <Card className="bg-black/50 border border-matrix-green/50 p-4 md:p-6">
                {isLoading && <div className="text-center py-4 text-matrix-green">Loading datasources...</div>}
                {error && <div className="text-center py-4 text-red-500">Error loading datasources: {error.message}</div>}
                ...
                {!isLoading && !error && (
                    <div className="space-y-4">
                        {datasources && datasources.length > 0 ? (
                                ...
                                >
                                    ...
                                    <div className="flex space-x-2 self-end md:self-center">
                                        <Button
                                            variant="outline"
                                            size="sm"
                                            onClick={() => { /* TODO: Open edit modal */ alert(`Edit ${ds.name}`); }}
                            ...
                            <div className="text-center py-4 text-matrix-green/70">No datasources configured yet.</div>
                        )}
                    </div>
                )}
            </Card>
