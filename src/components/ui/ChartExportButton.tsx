  ...
  return (
    <div className={`relative ${className}`}>
      <Button
        ...
        className="bg-primary/10 text-primary border border-primary/50 hover:bg-primary/20 h-10 px-3"
        title="Export Chart"
      >
        {isExporting ? (
          <div className="w-4 h-4 border border-primary border-t-transparent rounded-full animate-spin flex-shrink-0" />
        ) : (
      ...
      {showDropdown && (
        <>
          ...
          {/* Dropdown */}
          <div className="absolute right-0 top-full mt-2 w-48 bg-surface border border-border rounded-lg shadow-xl z-20 backdrop-blur-sm">
            <div className="py-2">
              {EXPORT_FORMATS.map((format) => (
                <button
                  ...
                  className="w-full flex items-center px-4 py-2 text-sm text-primary hover:bg-primary/10 transition-colors disabled:opacity-50"
                >
