...
export const ChartRenderer = forwardRef<ChartRendererRef, ChartRendererProps>(({
    ...
    if (isLoading) {
        return (
            <Card className="h-full flex items-center justify-center" style={{ background: 'rgba(0, 20, 0, 0.5)' }}>
    ...
    if (error) {
        return (
            <Card className="h-full flex items-center justify-center border border-red-500/50" style={{ background: 'rgba(20, 0, 0, 0.5)' }}>
        ...
        if (chartType === 'number') {
            ...
            return (
                <Card className="h-full flex flex-col" style={{ background: 'rgba(0, 20, 0, 0.5)' }}>
                    {title && <h3 className="text-md font-mono text-matrix-green px-4 pt-3 pb-1 text-center">{title}</h3>}
                        ...
                        <table className='w-full text-left text-xs'>
                            <thead>
                                <tr className='border-b border-matrix-green/50'>
                                    {keys.map(key => <th key={key} className='p-2 text-matrix-green font-normal sticky top-0 bg-black/80'>{key}</th>)}
                                </tr>
                            </thead>
                            <tbody>
                                {dataToRender.map((row, rowIndex) => (
                                    <tr key={rowIndex} className='border-b border-matrix-green/20 hover:bg-matrix-green/5'>
                                        {keys.map(key => <td key={`${rowIndex}-${key}`} className='p-2 text-matrix-green/80 whitespace-nowrap'>{String(row[key] ?? '')}</td>)}
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                    </div>
                    {dataToRender.length === 0 && <p className="text-matrix-green/70 text-sm text-center p-4">No data available.</p>}
                </Card>
            );
        }
    }
    ...
    return (
        <Card 
            ...
            ref={containerRef}
        >
            {title && <h3 className="text-md font-mono text-matrix-green px-4 pt-3 pb-1 text-center flex-shrink-0">{title}</h3>}
            {isLive && <div className={`absolute top-3 right-3 w-2.5 h-2.5 rounded-full ${isWsConnected ? 'bg-matrix-green animate-pulse shadow-[0_0_8px_#33ff33]' : 'bg-red-600 shadow-[0_0_8px_#ff0000]'}`} title={isWsConnected ? 'Live' : 'Disconnected'}></div>}
