...
// Memoized: token objects keep their identity across filter/sort changes, so
// only rows that actually enter/leave the list re-render.
const TokenRow = memo(({ token, onShowDetails }: TokenRowProps) => (
  <motion.tr
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
  ...
  >
    ...
    <td className="px-4 py-3 text-right">
      <div className="text-text-secondary">{new Date(token.launchDate).toLocaleDateString()}</div>
...
export const TokenTable = ({ tokens, onSort, onShowDetails }: TokenTableProps) => (
  <table className="w-full text-sm">
    ...
    <tbody>
      {tokens.length > 0 ? (
        tokens.map((token) => (
          <TokenRow key={token.id} token={token} onShowDetails={onShowDetails} />
        ))
      ) : (
        <tr>
